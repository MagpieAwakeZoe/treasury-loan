...
export default {
    ...
    methods: {
        goDetail (item) {
            ...
            this.$router.push({
                ...
                query: {orderId: item.orderId, repaymentId: item.repaymentId}
            });
        }
    },
    filters: {
    ...
    mounted () {
        this.$setBodyBack();
        Api.fsc.v1_repaymentApi_getOrderRepaymentRecordList({
            orderId: this.$route.query.orderId
        }).then((res) => {
            this.loading = false;
            this.list = res;
            this.$setBodyBack(this.list.length ? '#f2f2f2' : '#fff');
        }).catch(() => {
            this.loading = false;
