...
export default {
    ...
    methods: {
        ...
        editPickerInfo () {
            ...
            Api.fsc.v1_carPickUpApi_editPickerInfo({
                ...this.form,
                carPickUpId: this.$route.query.carPickUpId
            }).then(() => {
                this.$router.replace({path: '/pickup-detail', query: {carPickUpId: this.$route.query.carPickUpId}});
            });
        },
        validForm () {
        ...
        addNewPickupRequest () {
            ...
            Api.fsc.v1_carPickUpApi_addNewPickupRequest(arg).then((res) => {
                this.$router.replace({path: '/pickup-detail', query: {carPickUpId: res.carPickUpId}});
            });
        }
    },
    mounted () {
        if (this.$route.query.edit) { // 如果是修改提车人
            Api.fsc.v1_carPickUpApi_getPickupRequestInfo({carPickUpId: this.$route.query.carPickUpId}).then((res) => {
                let {carPickerName, estimatedPickDate, idCardNo, carPickerMobile} = res;
                this.form = Object.assign(this.form, {carPickerName, estimatedPickDate, idCardNo, carPickerMobile});
            });
        }
    }
};
